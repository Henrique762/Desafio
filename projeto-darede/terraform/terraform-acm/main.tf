...
resource "aws_acm_certificate_validation" "tls-validation" {
  certificate_arn         = aws_acm_certificate.tls.arn
  validation_record_fqdns = [for record in aws_route53_record.record-acm : record.fqdn]
}